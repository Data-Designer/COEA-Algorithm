function LS2_individual=local_search2(individual,user,item,investacount,requestacount,mutationrate)
...
for i=1:size(mutalocation)
    ...
    if mutated(temp)==0 && singleitem<requestacount(col)
        ...
        if mutated(temp)==1 && singleitem-investacount(u)>requestacount(col)
            mutated(temp)=0;
        end
    end
end
